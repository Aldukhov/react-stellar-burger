...
const RenderItemsOfType: React.FC<IRenderItemsProps> = ({
	burger,
	type,
	styles,
	data,
	...
	if (burger === 'ingrediens') {
		...
		ingrediens = filterType.map(element => {
			return <Ingredient key={element._id} styles={styles} element={element} />
		})
	} else {
		if (!data) {
			return null
			...
			ingrediens = data
				...
				.map((element, index) => (
					<ConstructorIngredient
						...
						element={element}
						index={findBun() ? index + 1 : index}
						moveItem={element.type === 'bun' ? null : moveItem}
	...
	return <section className={styles.items}>{ingrediens}</section>
}
