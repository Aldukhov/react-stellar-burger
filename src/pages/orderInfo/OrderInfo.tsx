...
const OrderInfo: React.FC<IOrderInfo> = ({ description }) => {
	...
	useEffect(() => {
		...
		if (!foundElement && Object.keys(data).length !== 0) {
			const orderElement = data.orders.find(
	...
	const fetchOrderData = async (): Promise<void> => {
		...
		if (result.success) {
			if (result.data.orders) {
				...
				console.error('There`s no order number!')
			}
		} else {
			console.error('Error when receiving order data:', result.error)
		}
	}
	...
	const returnElements = (
		...
		const ingredientsList = duplicates.map(ingredient => {
			...
			return (
				<li
					className={classNames(styles.orderInfo__ingredient, 'mb-6')}
					key={item._id}
				>
					<div className={styles.orderInfo__info}>
						<img
							src={item.image}
							className={classNames(styles.orderInfo__img, 'mr-4')}
							alt={item.name}
						/>
						<p
							className={classNames(
								styles['orderInfo__ingredient_name'],
					...
					<div className={styles['price-block']}>
						<p
							className={classNames(
								styles['orderInfo__ingredient_price'],
								'text text_type_digits-default mr-2'
							)}
						>
							{ingredient.count} x {item.price}
						</p>
				...
				<div className={styles[`orderInfo__${description}`]}>
					...
					<div className={styles.timestamp}>
						<p className={'text text_type_main-default text_color_inactive'}>
							<FormattedDate date={new Date(foundElement.updatedAt)} /> i-GMT+3
						</p>
						<div className={styles['price-block']}>
							<p
								className={classNames(
