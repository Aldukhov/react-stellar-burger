...
const OrderFeed: React.FC = () => {
    ...
    const columnCount = (orderName: string, customStyle?: string): JSX.Element[] | null => {

        const statusOfOrder: (IOrdersApi | undefined)[] = ordersStatus(orderName);
        const lastTwentyOrders: (IOrdersApi | undefined)[] = statusOfOrder.slice(0, 20);
      
        const ulArray: JSX.Element[] = [];
      
        const lastTwentyOrdersFiltered = lastTwentyOrders.filter(order => order !== undefined);
        ...
        if (lastTwentyOrdersFiltered.length > 0) {
          for (let i = 0; i < lastTwentyOrdersFiltered.length; i += 10) {
            ...
            for (let j = i; j < i + 10 && j < lastTwentyOrdersFiltered.length; j++) {
